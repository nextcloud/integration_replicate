...
<template>
	<div>
		...
		<img v-show="!isImageLoading && !failed"
			class="image"
			:src="src"
			:aria-label="t('integration_replicate', 'Generated image')"
			@load="isImageLoading = false"
			@error="onError">
		<span v-if="failed">
			{{ t('integration_replicate', 'The remote image cannot be fetched. Replicate might have deleted it.') }}
			<a :href="directLink" target="_blank" class="external">
				{{ t('integration_replicate', 'Direct image link') }}
			</a>
		</span>
	</div>
</template>
